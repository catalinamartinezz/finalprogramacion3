...
class Peliculas extends Component {
  ...
  componentDidMount() {
    ...
    const url = 'https://api.themoviedb.org/3/movie/popular?api_key=809187852af3a04706d10c0477580eec&page=1'
    fetch(url)
      .then((res) => res.json())
      .then(datos => {
        console.log(datos)
    ...
    return (
      <>
        <div className="botones1">
          <form>
            <input className="formu" type="search" name="search" placeholder="Buscar..." value={this.state.filterBy} onChange={(e) => { this.handleChange(e) }} />
          </form>
        </div>
        {this.state.filterBy === "" ? <>
          <div>
